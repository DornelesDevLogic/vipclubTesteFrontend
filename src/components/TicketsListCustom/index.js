...
const TicketsListCustom = (props) => {
  ...
  useEffect(() => {
    const count = includeGroups 
      ? ticketsList.length 
      : ticketsList.filter((ticket) => !ticket.isGroup).length;
    if (typeof updateCount === 'function') {
      updateCount(count);
    }
  }, [ticketsList, updateCount, includeGroups]);
          ...
          {ticketsList.length === 0 && !loading ? (
            ...
            <>
              {ticketsList
                .filter((ticket) => {
                  if (includeGroups) {
                    return true; // Mostra todos os tickets (conversas e grupos)
                  }
                  return !ticket.isGroup; // Mostra apenas conversas individuais
                })
                .map((ticket) => (
